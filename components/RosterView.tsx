    ...
    return (
        <div className="max-w-7xl mx-auto space-y-8">
             <div className="bg-white dark:bg-slate-800 p-4 rounded-lg shadow-md">
                <div className="flex justify-between items-center">
                    <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100 flex items-center">
                        <UsersIcon className="w-7 h-7 mr-3 text-sky-500"/>
            ...
            {partners.map(partner => {
                const programsForPartner = programs.filter(p => p.partnerId === partner.id);
                return (
                    <details key={partner.id} className="bg-white dark:bg-slate-800 rounded-lg shadow-md overflow-hidden" open>
                        <summary className="p-4 flex justify-between items-center cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-700/50">
                            <h3 className="text-xl font-semibold text-slate-800 dark:text-slate-100">{partner.name}</h3>
                             <span className="text-sm font-medium text-slate-500 dark:text-slate-400">
                                {roster.filter(c => programsForPartner.some(p => p.id === c.programId)).length} Client(s)
                             </span>
                        </summary>
                        <div className="p-4 sm:p-6 space-y-6 border-t border-slate-200 dark:border-slate-700">
                             {programsForPartner.map(program => {
